...
def theater_scrape():
    ...
    # Build the endpoint URL
    target_url = ('https://maps.googleapis.com/maps/api/geocode/json?'
                  'address={0}&key={1}').format(user_target, gkey)
    # Run a request to endpoint and convert result to json
    user_geo_data = requests.get(target_url).json()
    user_lat = user_geo_data["results"][0]["geometry"]["location"]["lat"]
    user_lng = user_geo_data["results"][0]["geometry"]["location"]["lng"]
    user_data.append({'user_lat': user_lat, 'user_long': user_lng})

    scrape_dict = {"Theaters": theater_data, "User_Data": user_data}
    with open('/static/scrape/result.json', 'w') as fp:
